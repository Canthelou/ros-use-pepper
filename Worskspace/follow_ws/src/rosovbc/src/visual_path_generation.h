...
class VisualPathGeneration
{

public:

    VisualPathGeneration()
        : m_it(m_nh), m_plotter(2, 1000, 600, 1300, 0, "Photometric and velocity evolution")
    {
        ...
        int px_marge_width = 65;
        int px_marge_height = 55;

        m_disp_curr.init(m_curr_im, px_marge_width, 0,"current image");
        m_disp_des.init(m_des_im, px_marge_width+m_width, 0,"desired image");
        m_disp_des_rot.init(m_des_rot_im, px_marge_width+m_width, px_marge_height+m_height, "desired rotated image");
        m_disp_diff.init(m_diff_im, px_marge_width+2*m_width, 0,"difference image");
        ...
        sstr_log<<m_logs_path<<"logfile_visual_path_generation.txt";
        ...
        m_image_sub = m_it.subscribe(camera_topic, 1, &VisualPathGeneration::imageCallback, this);
        ...
        m_ros_sub = m_nh.subscribe<geometry_msgs::Twist>(outputVelocityTopic, 1,
                                                         &VisualPathGeneration::robotVelocityCallback, this);

        /// callback timer if no signal for 10 seconds then quit the program
        m_timer = m_nh.createTimer(ros::Duration(10.0), &VisualPathGeneration::callbackTimer, this);

        initVisualPathGeneration();
    ...
    void initVisualPathGeneration()
    {
        m_logfile<<"call to initVisualPathGeneration"<<endl;
...
//        return;
        ...
        vpImage<unsigned char> tmp_curr2(m_height, m_width);
        ...
        if(m_b_move_forward && m_iter >=1)
        {
            //            m_logfile<<"#### Moving forward"<<endl;
            if (criterion_des_im_regular_rate())
            {
                m_des_im  = m_curr_im;
                build_and_save_desired_features();
                vpDisplay::display(m_des_im);
                vpImagePoint ip;
                // Display in overlay a green string
                ip.set_i( m_width/2 - 10 );
                ip.set_j( m_height/2 );

                str_des_im<<m_nb_des_im;

                vpDisplay::displayCharString(m_des_im, ip,
                                             str_des_im.str().c_str(),
                                             vpColor::green);
                vpDisplay::flush(m_des_im);

                // Create a color image
                vpImage<vpRGBa> Ioverlay ;
                vpDisplay::getImage(m_des_im, Ioverlay) ;
                vpImageConvert::convert(Ioverlay, m_des_im);
            ...
            //        image_difference(m_curr_im, m_des_im, m_diff_im);
            vpDisplay::display(m_diff_im);
            vpDisplay::flush(m_diff_im);
        }


        else if (m_b_move_backward)
        {
            m_iter_visual_servo++;
            display_current_image();

            m_logfile<<"#### Moving backward"<<endl;
            /// consider the first desired feautures after reverting the vectors
...
//                m_sId_backward       = m_vec_sId[m_index_desired_image];
//                m_Lsd_backward       = m_vec_Lsd[m_index_desired_image];
//                m_Hsd_backward       = m_vec_Hsd[m_index_desired_image];
//                m_diagHsd_backward   = m_vec_diagHsd[m_index_desired_image];
                ...
                double theta = find_best_angle(m_des_im, m_curr_im, center, step, m_logfile );
                ...
                rotate_image(m_des_im, center, theta, m_des_rot_im, m_logfile);
...
//                perform_visual_servo_iteration(m_curr_im, m_sI, m_sId_backward, m_Lsd_backward,
//                                               m_Hsd_backward, m_diagHsd_backward, m_lambda, m_mu,
//                                               m_current_error, m_vp_velocity);

                perform_robust_visual_servo_iteration(m_curr_im, m_sI, m_sId, m_Lsd,
...
//                }
                ...
                velocity.angular.z = -m_vp_velocity[1];

                        //                velocity.angular.z = vpMath::sign(m_vp_velocity[0])*m_vp_velocity[1];
                m_logfile<<"velocity"<<velocity.linear.x<<"\t"<<velocity.angular.z<<endl;
                m_ros_pub.publish(velocity);
//                plot_and_save_curves();
    ...
    {
        if ((m_iter%30 == 0) /*&& (((m_listened_vel.linear.x) != 0.0) ||  ((m_listened_vel.linear.z) != 0.0))*/)
        {
...
//        m_logfile<<"computed control law"<<endl;
    ...
    {
        if ((m_listened_vel.linear.x == 0.0) && (m_listened_vel.angular.z == 0.0) && (m_iter !=0))
        {
            //YA: the callback will be called every 10 seconds, but the following "if" should be called only once
            // any better idea is very welcome :)
            if(m_nb_backward == 0)
            {
                // if no velocity is received
                ROS_INFO("Timer callback triggered");
                write_data();
                this->m_b_move_forward = false;
                this->m_b_move_backward = false;
                ROS_INFO("Reverting vectors");
                reverse_vectors();
            }
            m_nb_backward++;
        }
